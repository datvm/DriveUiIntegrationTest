...
new class {
    ...
    #grpInit: HTMLFieldSetElement = document.querySelector(".grp-init")!;
    #grpIntegration: HTMLFieldSetElement = document.querySelector(".grp-integration")!;

    #txtAppUrl: HTMLInputElement = document.querySelector("#txt-url")!;

    constructor() {
        document.querySelector(".btn-create-client")?.addEventListener("click",
            () => void this.#createClient());

        document.querySelector("[data-simulate=open]")?.addEventListener("click",
            () => void this.#simulateAsync(true));
        document.querySelector("[data-simulate=new]")?.addEventListener("click",
            () => void this.#simulateAsync(false));
    ...
    #fillStoredOauth() {
        ...
        try {
            ...
            document.querySelectorAll<HTMLInputElement>("[data-oauth]").forEach((el) => {
    ...
    async #simulateAsync(open: boolean) {
    ...
    async #simulateOpenAsync(): Promise<string | undefined> {
        if (!this.#drive) { return; } // Should not happen
    ...
    async #simulateCreateAsync(): Promise<string | undefined> {
        if (!this.#drive) { return; } // Should not happen
